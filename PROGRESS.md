...
1. **"인천" 지역 항목 공급사·권역 정합성 재검토** (31번 항목에서 발견) — 기존 "인천"이 삼천리로 등록돼 있으나 인천 관내에는 삼천리와 별개 회사인 인천도시가스㈜가 다른 권역(서구·부평·계양·강화군 등)을 공급 중. "인천" 항목 수치(22.5084원/MJ)가 정확히 어느 회사/권역 대표값인지 재확인 필요 — 필요 시 "인천"을 삼천리 관할 권역(미추홀구 등)으로 재정의하고 인천도시가스 관할(서구·부평 등)은 별도 지역으로 분리하는 방향 검토
2. **애드센스 실제 심사 결과 확인** — 기술 점검(SDK 로드·ads.txt·콘솔에러) 완료(30번 항목), 슬롯 상태 `unfilled` 확인. 실제 심사 통과 여부는 Google 로그인 필요해 사용자가 애드센스 대시보드에서 직접 확인
3. **Search Console 대시보드 직접 확인** — sitemap 누락 버그는 수정 완료(19번 항목). Google 계정 로그인이 필요해 사용자 직접 확인
4. **도시가스 계산기 지역 12차 확장 후보**: 경기도 내 남은 소도시(안양·광명·의왕·군포·수원·안성·평택·오산·과천·하남·광주·이천·여주·구리·가평·양주·동두천·연천·포천 등)는 이미 6개사(삼천리/코원/예스코/대륜이엔에스/서울도시가스/인천도시가스) 모두 "경기도는 공급사 무관 동일 요금" 원칙이 확증되어, 추가해도 수치 확인 없이 확장 가능(우선순위는 사용자 판단)
