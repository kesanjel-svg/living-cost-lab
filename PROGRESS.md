...
3. **기존 `daegu` 항목 기본요금 정확도 재검토(우선순위 높음)**: 21번 항목에서 발견된 대구 기본요금 불일치(현재 950원 근사치 vs 협회 표 원본 900원/개별) 확인 후 수정, 이후 경산(대성에너지, 대구와 동일 공급사) 추가를 함께 진행
4. **도시가스 계산기 지역 8차 확장 후보**: 경기도 내 미확인 소도시(고양·용인·화성·파주·김포·안산·시흥 등 — 이미 "경기도는 공급사 무관 동일 요금" 원칙이 확인되어 추가 조사 우선순위는 낮음), 강원 원주 인접 횡성(원주와 동일 공급사·요금이라 이름만 추가하면 되는 저비용 후보), 평창(명성파워그린, 인구 적어 낮은 우선순위) 등 — 협회 표에 이미 나온 지역은 사실상 소진되어 다음 라운드부터는 협회 표에 없는 지역의 개별 조사가 필요할 가능성이 높음
