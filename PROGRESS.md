...
29. **연봉 실수령액 계산기 신규 구현 (완료, 2026-07-11, 계산기 4개→5개)**
   ...
   - 커밋 예정, push까지 자동 진행(사용자 승인: "추천순으로 작업을 진행해주고, push까지 자동으로 진행해줘")

### 🔜 다음 할 일
아래 중 우선순위는 사용자 확인 후 진행:
1. **애드센스 실제 심사 결과 대기/확인** — SDK 로드 구현 완료, 배포 반영됨. 콘텐츠량은 계산기 5 + 지원금 21 = 26개. 2026-07-11: 브라우저 확장(claude-in-chrome)이 이번 세션부터 연결되어 프로덕션 페이지 자체(스크립트 로드 여부 등) 기술적 확인은 가능해졌으나, 실제 광고 노출 여부·심사 통과 여부는 Google 계정 로그인이 필요한 애드센스 대시보드 확인이 필요해 여전히 사용자 직접 확인 필요(로그인 대행 불가)
2. **Search Console 대시보드 직접 확인** — sitemap 누락 버그는 수정 완료(19번 항목). Google 계정 로그인이 필요해 브라우저 확장 연결 여부와 무관하게 사용자 직접 확인 필요
3. **도시가스 계산기 지역 11차 확장 후보**: 이번 라운드(25번 항목)에서 보류한 **김포**(서울도시가스·인천도시가스 분할 공급 — 인천도시가스 쪽 실측 필요) 우선 후보. 그 외 경기도 내 남은 소도시(안양·광명·의왕·군포·수원·안성·평택·오산·과천·하남·광주·이천·여주·구리·가평·양주·동두천·연천·포천 등)는 이미 삼천리/코원/예스코/대륜이엔에스/서울도시가스 5개사 모두 "경기도는 공급사 무관 동일 요금" 원칙이 확증되어, 추가해도 수치 확인 없이 확장 가능(근거 확보 용이, 우선순위는 사용자 판단). 협회 요금표에 개별 수치가 게재된 지역은 사실상 모두 반영 완료라, 다음 라운드부터는 새 지역 발굴보다 기존 원칙 적용 지역 일괄 확장 또는 다른 작업(애드센스/Search Console 등) 우선을 검토
4. ~~이번 세션 커밋 push~~ — 2026-07-10 전부 push 완료(origin/master = deb2652), Vercel 자동 배포 반영
